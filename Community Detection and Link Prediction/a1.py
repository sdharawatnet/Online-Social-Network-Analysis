...
#
# Complete the **15** methods below that are indicated by 'to do'. I've provided some sample output to help guide your implementation.
...
def bfs(graph, root, max_depth):
    ...
    visiting_q = deque()
    visited_q = deque()
    node2distances = defaultdict()
    node2num_paths = defaultdict()
    node2parents = defaultdict()

    #Initializing root in next step.
    
    node2distances[root] = 0
    node2num_paths[root] = 1
    visiting_q.append(root)
    visited_q.append(root)
   ...
   # if len(visiting_q) == 0:
    ...
    while (len(visiting_q)!=0):
        current = visiting_q.popleft()
        visited_q.append(current)
        
        # More than maximum depth then ignore.
        if node2distances[current] >= max_depth:
            break
        else:
            for node in graph.neighbors(current):
                if not (node in visited_q):
                    
                    if node in visiting_q and node2distances[node] != node2distances[current]:
                        node2parents[node].append(current)
                        node2num_paths[node] = len(node2parents[node])
                    
                    if not (node in visiting_q):    
                        node2distances[node] = node2distances[current] + 1
                        node2parents[node] = [current]
                        node2num_paths[node] = len(node2parents[node])
                        visiting_q.append(node)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    # Initializing values of all the nodes = 1.0.
    
    for dist in node2distances:
        nodes[dist] = 1.0
    
    distances= sorted(node2distances.items(), key=lambda tup:(-tup[1], tup[0]))
    
    #Computing every node using bottom up approach.
    
    for d in distances:
        val = nodes[d[0]]
        if d[0] != root:
            parent_node = node2parents[d[0]]
            parent_len= len(parent_node)
            if (parent_len==0) :
                break
            
            if(parent_len != 1):
                edge_val = val/parent_len
                for tup in parent_node:
                    nodes[tup]= nodes[tup]+edge_val
                    a = tuple(sorted([d[0],tup]))
                    edges[a] = edge_val
            else:     
                
                nodes[parent_node[0]]= nodes[parent_node[0]] + val
                a  = tuple(sorted([parent_node[0],d[0]]))
                edges[a]= val
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in nodes:
        ...
        bottomup = bottom_up(n, node2distances, node2num_paths, node2parents)
        
        for edge in bottomup:
            
            if edge in apprbet:
                apprbet[edge] = apprbet[edge] + bottomup[edge]    
            else:
                apprbet[edge]= bottomup[edge]
            
    for edge in apprbet:
        apprbet[edge] = apprbet[edge]/2.0
...
def partition_girvan_newman(graph, max_depth):
    ...
    temp = graph.copy()
    appr_bet= approximate_betweenness(temp, max_depth)
    sort = sorted(appr_bet.items(), key= lambda tup:(-tup[1],tup[0][0],tup[0][1]))
    girnewman = list(sort)
    
    for val in range(0, len(girnewman)):
        temp.remove_edge(*girnewman[val][0])
        graph_clusters = list(nx.connected_component_subgraphs(temp))
        count=0
...
def cut(S, T, graph):
    ...
    for val in edges:
        ...
        if ((val[0] in set1) and (val[1] in set2)):
            counter= counter + 1
        elif((val[0] in set2) and (val[1] in set1)):
            counter = counter + 1
...
def norm_cut(S, T, graph):
    ...
    cut1= (cut(S,T,graph)/volume(S,graph))
    cut2 = (cut(S,T,graph)/volume(T,graph))
...
def score_max_depths(graph, max_depths):
    ...
    for max_val in max_depths:
        girvan = partition_girvan_newman(graph,max_val)
        score.append((max_val,norm_cut(girvan[0],girvan[1],graph)))
...
def make_training_graph(graph, test_node, n):
    ...
    pass
    temp = graph.copy()
    neighbors = sorted(graph.neighbors(test_node),key = lambda tup:tup[0])
    val = 0
    #if(val>n):
     #   val+=1
    while(val<n):
        temp.remove_edge(test_node,neighbors[val])
        val = val+1
...
def jaccard(graph, node, k):
    ...
    for val in nodes:
        if(val not in graph.neighbors(node)):
            neighbors2 = set(graph.neighbors(val))
            len1=len(neighbors & neighbors2)
            len2 =len(neighbors | neighbors2 )
            frac = (len1)/(len2)
            jaccard.append(((node,val),frac))

    sort = sorted(jaccard, key=lambda x: (-x[1], x[0][1]))
    sort.pop(0)
    return sort[:k]
...
def path_score(graph, root, k, beta):
    ...
    scores = []
    for node in temp:
        if(node not in graph.neighbors(root)):
            n2d =(beta ** node2distances[node])
            n2p= (node2num_paths[node])
            path_score =  (n2d) * (n2p)  #Used formula from the path_score.pdf
            scores.append(((root,node),path_score))
    if(scores!=0):
        sort = sorted(scores, key=lambda tup: (-tup[1], tup[0][1]))
        sort.pop(0)
...
def evaluate(predicted_edges, graph):
    ...
    #Getting value for every edge 
    
    pred_edges = len(predicted_edges)
    common_edges= 0
   # if(common_edges=0)

    for val in predicted_edges:
        if(graph.has_edge(val[0],val[1])):
            common_edges = common_edges + 1

    return common_edges / pred_edges
